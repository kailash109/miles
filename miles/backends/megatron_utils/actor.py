...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def prepare_training_engine_worker(self, engine_cfg: dict) -> dict:
        """Build this worker's plan executor over the already-initialized model.
    ...
    def execute_train_step_plan(self, plan):
        ...
        self._current_plan_id = getattr(plan, "plan_id", None)
        try:
            return self._plan_executor.execute(plan)
        finally:
            self._current_plan_id = None

    def training_engine_health(self):
        from miles.training_engine.results import WorkerHealth

        rank = dist.get_rank() if dist.is_initialized() else 0
        return WorkerHealth(
            rank=rank,
            current_plan_id=getattr(self, "_current_plan_id", None),
            state="ready" if getattr(self, "_plan_executor", None) is not None else "uninitialized",
        )
